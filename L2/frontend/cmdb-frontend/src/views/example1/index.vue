  ...
  <div class="example1-wrapper" >
    <div class="fr clearfix mb15">
      <bk-form form-type="inline">
        ...
        <bk-form-item label="主机名">
          <bk-input placeholder="支持模糊查询" v-model="hostName" />
        </bk-form-item>
        <bk-form-item label="主机IP">
          <bk-input placeholder="支持模糊查询" v-model="hostIp" />
        </bk-form-item>
        <bk-form-item label="备份人">
          <bk-input placeholder="备份维护人" v-model="bakOperator" />
        </bk-form-item>
        <bk-form-item>
    ...
    <bk-table style="margin-top: 15px;"
      ...
      <bk-table-column label="主机名" prop="bk_host_name" />
...
export default {
  ...
  methods: {
    ...
    async searchHosts(){
      if (!this.biz_id) {
        this.$bkMessage({ theme: 'warning', message: '请先选择业务', offsetY: 80 });
        return;
      }
      let query_data = {
      ...
      if (this.hostName) {
        query_data["bk_host_name"] = this.hostName;
      }
      if (this.hostIp) {
        query_data["bk_host_innerip"] = this.hostIp;
      }
      if (this.bakOperator) {
        query_data["bk_bak_operator"] = this.bakOperator;
      }
      ...
      this.pagination.count = host_res.data.count || this.host_list.length
    },
