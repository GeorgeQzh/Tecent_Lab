...
def get_behavior_summary(request):
    """
    查询用户行为统计，供前端兜底展示与 BKVision 数据源校验。
    """
    total = ApiRequestCount.objects.count()
    by_path = list(
        ApiRequestCount.objects.values("path")
        .annotate(count=Count("id"), avg_cost=Avg("cost_ms"))
        .order_by("-count")[:20]
    )
    by_user = list(
        ApiRequestCount.objects.values("username")
        .annotate(count=Count("id"), avg_cost=Avg("cost_ms"))
        .order_by("-count")[:20]
    )
    trend = list(
        ApiRequestCount.objects.annotate(day=TruncDate("created_at"))
        .values("day")
        .annotate(count=Count("id"))
        .order_by("day")
    )
    for item in trend:
        item["day"] = item["day"].strftime("%Y-%m-%d") if item["day"] else ""
...
def get_dashboard_config(request):
    """
    返回 BKVision 发布仪表盘 iframe 地址。
    部署时可在环境变量 BK_VISION_DASHBOARD_URL 中配置实际发布地址。
    """
    return JsonResponse(
        {
            "result": True,
            "code": WEB_SUCCESS_CODE,
            "data": {
                "iframe_url": os.getenv(
                    "BK_VISION_DASHBOARD_URL",
                    "https://apps.ce.bktencent.com/bk-vision/embed/",
                )
            },
        }
    )
